	...
	\section{Transforms}
    	...
    	\subsubsection{curl}
     	...
     	\section{Stokes' Theorem}
     		...
     		Stokes' theorem is a special case of the generalized Stokes' theorem. In particular, a vector field on $\R^3$ can be considered as a 1-form in which case its curl is its exterior derivative, a 2-form. \\
     		
     		Let $S$ be a smooth oriented surface in $\R^3$ with boundary $\partial S$. If a vector field {$\F \xyz = (P\xyz, Q\xyz, R\xyz)$ is defined and has continuous first order partial derivatives in a region containing $S$, then 
     		\begin{equation}
